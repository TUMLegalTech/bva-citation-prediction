...
class DataPreprocessor:
    ...
    def process_partition_txt_files(self, input_dir, output_dir, partition_ids):
        fnames = [fname + ".txt" for fname in partition_ids]
        args = [(os.path.join(input_dir, fname),
                 os.path.join(output_dir, fname[:-3]+'json'))
                    for fname in fnames]
                
        pool = Pool(os.cpu_count())
        pool.starmap(self.process_single_txt_file, args)
